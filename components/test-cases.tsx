...
export default function TestCases({
  ...
  const update = (patch: Partial<TestCase>) =>
    setCases((list) => list.map((c) => (c.id === active.id ? { ...c, ...patch } : c)));
  const run = async () => {
    setBusy(true);
    setError("");
    setResults([]);
    ...
    <section className="panel test-panel">
      <div className="panel-heading">
        <div>
          ...
          <p>Real inputs. Real execution. Run every solution against the same cases.</p>
        </div>
        <button className="button primary small" disabled={busy} onClick={run}>
          {busy ? <LoaderCircle size={15} className="spin" /> : <Play size={14} />}Run{" "}
          {cases.length} cases
        </button>
      </div>
      <div className="test-content">
        <div className="test-case-list">
          {cases.map((c) => (
            ...
            >
              <span className={`case-indicator ${c.kind}`} />
              <span>
                {c.name}
                <small>{c.kind} case label</small>
              </span>
              <ChevronDown size={13} />
            </button>
          ))}
        ...
        <div className="test-editor">
          <div className="test-form-row">
            ...
            {cases.length > 1 && (
              <button
                ...
                onClick={() => setCases((list) => list.filter((c) => c.id !== active.id))}
              >
                <Trash2 size={15} />
              </button>
            )}
          ...
          <p className="form-hint">
            Library algorithms read: n, then n integers, then an optional target. Sorting outputs
            values; searches output a zero-based index or −1. For custom programs, use their stdin
          ...
          <details>
            ...
            <label className="runner-timeout">
              ...
              <input
                ...
                value={timeout}
                onChange={(e) =>
                  setTimeoutValue(Math.max(0.1, Math.min(5, Number(e.target.value) || 2)))
                }
              />
            </label>
            {algorithms.map((a) => (
      ...
      {results.length > 0 && (
        <div className="run-results">
          ...
          <p className="form-hint">
            Process wall time, including interpreter / JVM startup. Compilation and container
            startup excluded. Tiny cases are noisy; these measurements do not calibrate the estimate
            curves.
          </p>
                ...
                {results.map((r, i) => (
                  <tr key={i}>
                    <td>
                      ...
                      <small className="table-language">{r.caseName}</small>
                    </td>
                    <td>
                      <strong>{r.durationMs.toFixed(2)} ms</strong>
                      <div
                        className="measured-bar"
                        style={{
                          width: `${Math.max(3, Math.min(100, (r.durationMs / Math.max(...results.map((v) => v.durationMs), 1)) * 100))}%`,
                          background: r.color,
                        }}
                      />
                    </td>
                    <td>
                      <span
                        className={`status-pill ${r.status === "ok" && r.passed !== false ? "pass" : "tle"}`}
                      >
                        {r.status === "tle"
                          ? "TLE"
                    ...
                    <td>
                      <pre className="output-preview">{r.stderr || r.stdout || "(no output)"}</pre>
                    </td>
                  </tr>
                ))}
              </tbody>
