...
export const algorithmSchema = z.object({
  ...
  model: caseModel,
  factors: z.object({ best: cost, average: cost, worst: cost }),
  overhead: z.number().min(0).max(1e9),
  space: z.string().max(100),
  explanation: z.string().max(2000),
  conditions: z.object({
    best: z.string().max(1000),
    average: z.string().max(1000),
    worst: z.string().max(1000),
  }),
  origin: z.enum(["preset", "heuristic", "manual"]),
});
export const experimentSchema = z.object({
  name: z.string().trim().min(1).max(100),
  algorithms: z
    .array(algorithmSchema)
    .min(1)
    .max(6)
    .refine(
