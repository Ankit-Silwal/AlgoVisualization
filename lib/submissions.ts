import { wrapCNodes, wrapJavaScript } from "./node-adapters";
...
export function detectEntrypoint(code: string, language: string): string | undefined {
  if (language === "JavaScript")
    return (
      code.match(/(?:function\s+|(?:const|let|var)\s+)([A-Za-z_$][\w$]*)\s*(?:\(|=)/)?.[1] ||
      code.match(/class\s+Solution\s*\{\s*(\w+)\s*\(/)?.[1]
    );
  if (language === "Python") return code.match(/def\s+(?!__)(\w+)\s*\(/)?.[1];
  if (language === "Java")
    return code.match(/public\s+(?:static\s+)?[\w<>\[\], ?]+\s+(?!main\b)(\w+)\s*\(/)?.[1];
  return code.match(
    /(?:^|\n)\s*(?:struct\s+(?:ListNode|TreeNode)|int|long|double|float|bool|char|void)(?:\s*\*)?\s*(?!main\b)(\w+)\s*\(/,
  )?.[1];
}
...
function wrapJava(source: string, method: string, payload: string, jsonInput: boolean): string {
  ...
  public static void main(String[] ignored)throws Exception {List<?> values=(List<?>)new Json(String.join("",new String[]{${(payload.match(/[\s\S]{1,8000}/g) || [""]).map((part) => JSON.stringify(part)).join(",")}})).read();Method chosen=null;for(Method m:Solution.class.getDeclaredMethods())if(m.getName().equals(${JSON.stringify(method)})&&m.getParameterCount()<=values.size()){if(chosen!=null)throw new IllegalArgumentException("Overloaded methods need a unique entrypoint name");chosen=m;}if(chosen==null)throw new IllegalArgumentException("Method not found");chosen.setAccessible(true);Object[] a=new Object[chosen.getParameterCount()];for(int i=0;i<a.length;i++)a[i]=convert(values.get(i),chosen.getParameterTypes()[i]);Object result=chosen.invoke(Modifier.isStatic(chosen.getModifiers())?null:new Solution(),a);if(chosen.getReturnType()==void.class&&a.length>0)result=a[0];String out=encode(result);System.out.println(${jsonInput ? "out" : 'out.startsWith("[")?out.substring(1,out.length()-1).replace(","," "):out'});}
}`;
}
